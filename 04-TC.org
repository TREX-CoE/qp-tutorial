...
cd qp2
# switch to the branch of TC development
...
* TC CIPSI
  ...
  consider the same number of ~n_det_max~ as before:
...
exit
  ...
  [[./images/extrap_Etc.pdf]]


** CIPSI vs TC-CIPSI

  Using gnuplot with the files ~data~ (obtained in the previous calculation 
  of CIPSI with HF orbitals) and ~data_tc~ together with the following commands
